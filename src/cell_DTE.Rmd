...
#lncRNAs$qval = p.adjust(lncRNAs$pval, method = "fdr")
#lncRNAs = lncRNAs[lncRNAs$qval <= 0.05, ]
...
mat_breaks = c(min(mat, na.rm = TRUE), -3, -1,  0,  1, 3, 6 ,max(mat, na.rm = TRUE))
...
pheatmap(mat, cluster_rows = FALSE, cluster_cols = TRUE, labels_row = "", breaks = mat_breaks, color = div_cols, legend_breaks = c(-4, -2,0, 2, 4,6, max(mat, na.rm = TRUE)), legend_labels = c(-3, -1, 0, 1, 3,6, "L2FC\n") ) 
p
dev.off()
rm(mat2)

cyfracs = c()
for ( i in which(df$l2fc < 0)){
  if (sum(!is.na(mat[i, ])) > 1){
  cyfracs = c(cyfracs, sum(mat[i, ] <= 0, na.rm = TRUE)/sum(!is.na(mat[i, ] )) *100)
  }
}
...
for ( i in which(df$l2fc >  1)){
  if (sum(!is.na(mat[i, ])) > 1){
  nucfracs = c(nucfracs, sum(mat[i, ] > 1, na.rm = TRUE)/sum(!is.na(mat[i, ] )) *100)
  }
}
...
table(rowMeans(mat, na.rm = TRUE) < -1)
...
mart = useMart("ensembl", dataset = "hsapiens_gene_ensembl")
map = getBM(mart = mart, attributes = c("ensembl_transcript_id","transcript_biotype", "chromosome_name"), filters = "ensembl_transcript_id", values = rownames(df))
...
df$lincRNA = FALSE
df$lincRNA[df$transcript_biotype == "lincRNA"] = TRUE

df$antisense = FALSE
df$antisense[df$transcript_biotype == "antisense_RNA"] = TRUE
