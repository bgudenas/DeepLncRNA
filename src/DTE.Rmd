...
## Modify Sleuths basic_filter function to change req of 5 reads in 47% of samples to ==> 5 reads in 4 samples (4/88) to pick up more cell type specific lncRNAs
loose_filter <- function(row, min_reads = 5, min_prop = 0.04545455) {
...
## Check RLE and PCA  plots after Normalization and filtering
Expr = so$obs_norm_filt %>%  dplyr::select(target_id, sample, est_counts)
...
plotRLE(Emat, col = as.numeric(as.factor(s2c$condition))+2 , xlab = "Samples", ylab = "Relative Log Expression", outline = FALSE, ylim =c(-6,6))
legend(x = "topright",legend = c("Cytosol","Nuclear"), fill=c("green","blue"))

plotPCA(Emat, cex =2, col = as.numeric(as.factor(s2c$condition)), k=3, pch = as.numeric(as.factor(s2c$Lib)))
...
```{r RUVg}
# get raw counts
Expr = so$obs_raw %>%  dplyr::select(target_id, sample, est_counts)
Emat = tidyr::spread(Expr, key  = sample, est_counts)
...
# Code adapted from RUVSeq vignette
filter <- apply(Emat, 1, function(x) length(x[x>2])>=2) ##remove non-expressed genes (2 countts in 2 samples)
filtered <- Emat[filter,]
trans <- rownames(filtered)[grep("^ENS", rownames(filtered))]
spikes <- rownames(filtered)[grep("^ERCC", rownames(filtered))]

plotRLE(filtered, outline=FALSE, ylim=c(-6, 6), col=as.numeric(as.factor(s2c$condition))+2)
plotPCA(filtered, col=as.numeric(as.factor(s2c$condition))+2, cex=1.2, k=3, pch = as.numeric(as.factor(s2c$Lib)))
...
plotRLE(set1, outline=FALSE, ylim=c(-6, 6), col= as.numeric(x)+2)
...
```{r heatmap}
RNA_mat = so_RNA$obs_norm %>%  dplyr::select(target_id, sample, tpm)
RNAdf = tidyr::spread(RNA_mat, key  = sample, tpm)

IDs = unlist(lapply(strsplit(RNAdf$target_id, "\\."), "[[", 1))

defilt = match(IDs, rownames(DElncRNAs))

RNAdf = RNAdf[!is.na(defilt), ]

nopoly_mat = so_nopolyA$obs_norm %>%  dplyr::select(target_id, sample, tpm)
nopolydf = tidyr::spread(nopoly_mat, key  = sample, tpm) %>% filter(!is.na(defilt))   

poly_mat = so_polyA$obs_norm %>%  dplyr::select(target_id, sample, tpm)
polydf = tidyr::spread(poly_mat, key  = sample, tpm) %>% filter(!is.na(defilt))   

rm(RNA_mat, nopoly_mat, poly_mat)
annot = as.data.frame(so_RNA$sample_to_covariates[ ,c(2)])
rownames(annot) = so_RNA$sample_to_covariates$sample
pheatmap(RNAdf[ ,-1], annotation_col = annot,  scale = "row", cluster_rows = FALSE, labels_row = "",annotation_names_col = FALSE)

annot = as.data.frame(so_nopolyA$sample_to_covariates[ ,c(2,3)])
rownames(annot) = so_nopolyA$sample_to_covariates$sample
annot = annot[order(annot$condition, annot$cell), ]
nopolydf = nopolydf[ , c(1, match( rownames(annot), colnames(nopolydf) ))]
pheatmap(nopolydf[!is.na(DE_match) ,-1], annotation_col = annot,  scale = "row", cluster_rows = FALSE, labels_row = "",annotation_names_col = FALSE, cluster_cols = FALSE)

annot = as.data.frame(so_polyA$sample_to_covariates[ ,c(2,3)])
rownames(annot) = so_polyA$sample_to_covariates$sample
annot = annot[order(annot$condition, annot$cell), ]
polydf = polydf[ , c(1,match( rownames(annot), colnames(polydf) ))]
pheatmap(polydf[ ,-1], annotation_col = annot,  scale = "row", cluster_rows = FALSE, labels_row = "",annotation_names_col = FALSE, cluster_cols = FALSE)
```
...
#colnames to drop
# drop_cols = which(colSums(Supervised[ ,5:ncol(Supervised)]) == 0)
