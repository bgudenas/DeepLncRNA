...
lncRNAs = readRDS("./DE_lncRNAs.rds")
inTrain = sample(1:nrow(lncRNAs), .7*nrow(lncRNAs))

genome = readRDS("./genomic_lncRNAs.rds")
...
Kmer_filt = function(df){
  ...
  for ( i in 6:ncol(df)) {
    vals = tidy(cor.test(df[ , i], df$b, method = "spearman"))$p.value
    # vals = cor(df[ , i], df$b, method = "spearman")
    test_vals = c(test_vals, vals )
  }
...
Supervised = Kmer_add(lncRNAs, k = 8)
filt = Kmer_filt(Supervised[inTrain, ])
Supervised = Supervised[ , filt]
saveRDS(Supervised, "./Supervised_kmer.rds")

genome = genome[genome$transcript_length >= 200, ]
Unsupervised = Kmer_add(genome, k = 8)
Unsupervised = Unsupervised[ , filt]
saveRDS(Unsupervised, "./Unsupervised_kmer.rds")
```
...
## Motifs are in units of RNA so we must complement to DNA (A -> T, C -> G, G -> C, U -> A)
        ...
        for (j in 1:nrow(df)){
        seq = df$cdna[j]
        counts = countPWM(mot, seq, min.score = "80%") + countPWM(mot, reverse(seq), min.score = "80%")
        counts = counts/df$transcript_length[j]
...
set.seed(54321)
# Supervised$Localized = "Nuclear"
# Supervised$Localized[Supervised$b < -1] = "Cytosolic"
# Supervised = Supervised[!(Supervised$b > -1 & Supervised$b < 1), ]
