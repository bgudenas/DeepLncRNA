...
filespath = file.path(getwd(), "/Data/Results", s2c$sample)
...
samples = dplyr::filter(samples, File.accession != "ENCFF901DUO" & File.accession != "ENCFF792JEE")
write.csv(samples, "./Data/Meta/Sample_data.csv")
...
lnc_match = match(rownames(joined_df), lncRNAs$transcript_id)
joined_df$transcript_biotype = lncRNAs$transcript_type[lnc_match]
DElncRNAs = joined_df[!is.na(joined_df$transcript_biotype), ]
joined_df = joined_df[is.na(joined_df$transcript_biotype), ]

mart = useMart("ensembl", dataset = "hsapiens_gene_ensembl")

map = getBM(mart = mart, attributes = c("ensembl_transcript_id","transcript_biotype"), filters = "ensembl_transcript_id", values=rownames(joined_df))
joined_df$transcript_biotype = map$transcript_biotype[match( rownames(joined_df), map$ensembl_transcript_id) ]
...
library(gridExtra)
lncnum = data.frame("LncRNAs" = nrow(DElncRNAs), Nuclear = sum(DElncRNAs$b > 3), Cytosol = sum(DElncRNAs$b < -1), row.names = NULL)
grid.table(lncnum, vp = grid::viewport(x = 0.4, y = 0.9), rows = NULL, theme = ttheme_default(base_size = 16))
dev.off()

seq = biomaRt::getSequence(id = rownames(DElncRNAs), type="ensembl_transcript_id", seqType = "cdna", mart = mart)
DElncRNAs$cdna = seq$cdna[match(rownames(DElncRNAs), seq$ensembl_transcript_id)]
#lncRNAs = lncRNAs[, -2]
